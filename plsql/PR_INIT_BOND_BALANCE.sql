...
BEGIN
  I_BOND_BALANCE.BIZ_DATE        := '';  -- 영업일자(PK)
  I_BOND_BALANCE.FUND_CODE       := '';  -- 펀드코드(PK)
  I_BOND_BALANCE.BOND_CODE       := '';  -- 종목코드(PK)
  I_BOND_BALANCE.BUY_DATE        := '';  -- 매수일자(PK)
  I_BOND_BALANCE.BUY_PRICE       := 0;   -- 매수단가(PK)
  I_BOND_BALANCE.BALAN_SEQ       := 0;   -- 잔고일련번호(PK)
  ...
  I_BOND_BALANCE.DAMAGE_YN       := 'N'; -- 손상여부(Y/N)
