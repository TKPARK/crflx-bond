...
BEGIN
  I_BOND_TRADE.TRD_DATE           := '';  -- 거래일자(PK)
  I_BOND_TRADE.TRD_SEQ            := 0;   -- 거래일련번호(PK)
  ...
  I_BOND_TRADE.CANCEL_YN          := 'N'; -- 취소여부(Y/N)
  I_BOND_TRADE.TRD_TYPE_CD        := '';  -- 매매유형코드(1.인수, 2.직매수, 3.직매도, 4.상환)
  I_BOND_TRADE.GOODS_BUY_SELL_SECT:= '';  -- 상품매수매도구분(1.상품매수, 2.상품매도)
  I_BOND_TRADE.STT_TERM_SECT      := '';  -- 결제기간구분(0.당일, 1.익일)
  I_BOND_TRADE.SETL_DATE          := '';  -- 결제일자
  I_BOND_TRADE.EXPR_DATE          := '';  -- 만기일자
  I_BOND_TRADE.EVENT_DATE         := '';  -- 이벤트일
